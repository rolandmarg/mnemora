...
Create `infrastructure/samconfig.toml` file or use `sam deploy --guided` to set parameters:
...
### Lambda Timeout
- Increase timeout in `infrastructure/template.yaml` (max 15 minutes)
...
- Set up CloudWatch Dashboard (see [MONITORING.md](./MONITORING.md))
- Review alert types and severity levels (see [ALERTING_GUIDE.md](./ALERTING_GUIDE.md))
- Verify SNS email/SMS subscriptions are confirmed
- Set up CI/CD pipeline
- Migrate to WhatsApp Cloud API (see [MIGRATION_GUIDE.md](./MIGRATION_GUIDE.md))
